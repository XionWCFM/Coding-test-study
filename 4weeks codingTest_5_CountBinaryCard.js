...
function solution(n) {
    let answer = ''
    for(i=1 ; i <= n; i++) {
        answer += i.toString(2).toString()
    }
    return answer.split('').filter(ele => ele == 1).length
}

/*
접근방법

결국 1의 갯수를 구하기만 하면 되는 문제라고 생각했습니다.

1. 1~n까지의 케이스동안 for문을 돌린다.
2. answer 변수에 이진수를 문자열형태로 누적시킨다.
3. filter()를 이용해 1만 남겨주고 1의 갯수를 리턴시켜준다.
